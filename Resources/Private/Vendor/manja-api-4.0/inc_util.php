...
$a = [&$_FILES, /*&$_ENV,*/&$_GET, &$_POST, /*&$_COOKIE,&$_SERVER,*/&$_REQUEST];
foreach ($a[0] as &$r) {
    $a[] = [&$r['name'], &$r['type']];
}
unset($r);
...
for ($i=1; $i<$len; ++$i) {
    foreach ($a[$i] as &$r) {
        if (is_array($r)) {
            ...
            $r = mj_normalize_unicode_input($r);
        }
    }
    unset($r);
    unset($a[$i]);
...
class mjException extends Exception
{
    ...
    public function __construct($message, $details='', $previous=null)
    {
        ...
        if (!is_string($details)) {
            $details = 'details JSON=' . mj_json_encode($details);
        }
...
class mjHTTPException extends mjException
{
    ...
    public function __construct($http_code, $message='', $details='', $previous=null, $http_allow_methods=['GET', 'HEAD'])
    {
    ...
    public function sendHTTPResponseHeaders()
    {
        ...
        if ($this->http_code===405) {
            header('Allow: ' . implode(', ', $this->getHTTPAllowMethods()));
        }
    }
...
{
    if ($message===null) {
        $message = mjHTTPException::getHTTPMessageForCode($code);
    }
...
function http_cache_control_headers($directive='off', $max_age=0, $custom_last_modified_time=null, $custom_expires_time=null, $custom_cache_tag=null)
{
    ...
    case 'public':
        if ($max_age>=0) {
            header('Cache-Control: ' . $directive . ', max-age=' . (int)$max_age);
        } else {
            header('Cache-Control: ' . $directive . ', no-cache, max-age=0');
        }
        break;
    }
    $now = time();
...
function http_check_modified($doc_mtime, $doc_etag)
{
    ...
    if ($got_if_none_match || ($got_if_none_match===null && !http_check_if_modified_since($doc_mtime))) {
        return false;
    }
    return true;
}
...
function http_check_if_none_match($doc_etag)
{
    ...
    if ($tags===true) {
        return true;
    } // "If-None-Match: *" => match "ANY" ressource
...
function _mj_parse_config_file($path, $die_on_error)
{
    if (!is_file($path)) {
        if ($die_on_error) {
            die('configuration file not found or not readable: ' . $path);
        }
        return false;
    }
    ...
    if (!is_array($config) || count($config)===0) {
        if ($die_on_error) {
            die('syntax error in configuration file: ' . $path);
        }
        return false;
    }
    return $config;
...
function _mj_merge_config_arrays($config1, $config2)
{
    ...
    foreach (array_keys($config2+$config1) as $sk) {
        $c1 = isset($config1[$sk]) ? $config1[$sk] : [];
        $c2 = isset($config2[$sk]) ? $config2[$sk] : [];
...
function mj_parse_config($dir, $fn, $die_on_error=true)
{
    // read default config
    $config1_count = 0;
    if (($config1=_mj_parse_config_file($dir . '/defaults.' . $fn, false))===false) {
        $config1 = [];
    } else {
        $config1_count = count($config1);
    }
...
function mj_get_localized_ini_value($ini, $section, $key, $locale, $language, $default)
{
    if (isset($ini[$section])) {
        return mj_get_localized_ini_value2($ini[$section], $key, $locale, $language, $default);
    }
    return $default;
}
...
{
    if (isset($s[$key . '_' . $locale])) {
        return $s[$key . '_' . $locale];
    }
    if (isset($s[$key . '_' . $language])) {
        return $s[$key . '_' . $language];
    }
    if (isset($s[$key])) {
        return $s[$key];
    }
...
function mj_generate_password($passwords_cfg, $type)
{
    ...
    for ($i=0; $i<$length; ++$i) {
        $pass .= mb_substr($chars_alpha, mt_rand(0, $chars_alpha_len-1), 1, 'UTF-8');
    }
    // add numeric chars
...
{
    if (!isset($passwords_cfg['enable_validation']) || $passwords_cfg['enable_validation']!=='yes') {
        return true;
    }
    ...
    for ($i=0; $i<$pass_len; ++$i) {
        ...
        if (mb_strpos($chars_alpha, $c, 0, 'UTF-8')!==false) {
            $cnt_alpha ++;
        } elseif (mb_strpos($chars_numeric, $c, 0, 'UTF-8')!==false) {
            $cnt_numeric ++;
        } elseif (mb_strpos($special_chars, $c, 0, 'UTF-8')!==false) {
            $cnt_special ++;
        } else {
            $cnt_unknown ++;
        }
...
function mj_pluralize($e)
{
    $l = strlen($e);
    if ($l>1 && $e[$l-1]==='y') {
        if ($l>2 && ($e[$l-2]==='a' || $e[$l-2]==='e' || $e[$l-2]==='i' || $e[$l-2]==='o' || $e[$l-2]==='u')) {
            return $e . 's';
        }	// e.g. day->days, key->keys, ?iy->?iys, boy->boys, guy->guys
...
function mj_singularize($e)
{
    ...
    if ($l>1 && $e[$l-1]==='s') {
        ...
        if ($l>3 && $e[$l-3]==='i' && $e[$l-2]==='e') {		// e.g. categories->category, dependencies->dependency, ...
...
{
    if (($p=strrpos($str, $delimiter))!==false) {
        return [substr($str, 0, $p), substr($str, $p+1)];
    }
    return [$str];
}
...
if (!function_exists('mj_mergesort')) {
    function mj_mergesort(&$array, $cmp_function)
    {
        ...
        $a1end = end($array1);
        if (call_user_func_array($cmp_function, [&$a1end, &$array2[0]]) < 1) {
        ...
        while ($ptr1 < $ca1 && $ptr2 < $ca2) {
            if (call_user_func_array($cmp_function, [&$array1[$ptr1], &$array2[$ptr2]]) < 1) {
                $array[] = $array1[$ptr1++];
            } else {
                $array[] = $array2[$ptr2++];
            }
...
function get_localized_string2($val, $region, $default_region=null)
{
    ...
    if (is_array($val)) {
        if (isset($val[$region])) {
            return (string)$val[$region];
        }
        if ($default_region!==null && isset($val[$default_region])) {
            return (string)$val[$default_region];
        }
        if (isset($val[''])) {
            return (string)$val[''];
        }
