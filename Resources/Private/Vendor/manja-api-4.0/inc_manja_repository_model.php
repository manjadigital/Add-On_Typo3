...
class MjCPath
{
    ...
    {
        return new self(array_slice($this->segments, 0, $nsegments));
    }
...
abstract class MjCNode
{
    ...
    public function __construct(MjCRepository $repository, $attributes=[])
    {
...
class MjCDocument extends MjCNode
{
    ...
    public function GetContent()
    {
        ...
        if ($result!==false) {
            return $server->get_payload($result, true);
        }
    }
    ...
    public function GetStream()
    {
        ...
        if ($result!==false) {
            return $server->get_stream($result, true);
        }
    }
    ...
    public function SendRenditionToClient($par)
    {
        $p = explode('_', $par);
        if (count($p)<2) {
            throw new MjCInvalidArgumentException('SendRenditionToClient called with incomplete arguments');
        }
        $server = $this->repository->GetServer();
        ...
        if (false) {
            ...
            if (($x=strrpos($filename, '.'))!==false) {
                $filename = substr($filename, 0, $x) . '_' . $p[0] . 'x' . $p[1] . '.' . $sfx;
            }
    ...
    public function DeleteSelf($rebuild_temp_file_references=true)
    {
        ...
        if (!function_exists('mjwsu_media_delete')) {
            require __DIR__ . '/inc_manja_web_server_util.php';
        }
        $result = mjwsu_media_delete(null, $this->repository->GetServer(), $in_out_media_ids, $rebuild_temp_file_references, true);
        if ($result===false || !isset($result['media_ids'])) {
            return false;
        }
        $deleted_ids = $result['media_ids'];
        if (!in_array($this->document_id, $deleted_ids)) {
            return false;
        }
        if ($parent_node!==null) {
    ...
    {
        $meta = [ '1'=>[$new_name] ];
        $tmp = $this->repository->GetServer()->MediaUpdate($this->document_id, $meta, []);
        if ($tmp===false) {
            return false;
        }
    ...
    public function MoveToPath($trg_path, $new_name=null, $overwrite_mode=null)
    {
        ...
        if (($target_existing_node=$new_parent_node->GetChildByPathSegment($trg_segment_name))!==null) {
            ...
            // continue below ...
            } elseif ($overwrite_mode==='integrate-into-target') {
...
class MjCFolder extends MjCNode
{
    ...
    public function GetDocuments($offs=0, $count=2147483637)
    {
        ...
        $mlmeta = isset($media_list['meta']) ? $media_list['meta'] : [];
        foreach ($media_list['media_ids'] as $cmid) {
            $doc_meta = isset($mlmeta[$cmid]) ? $mlmeta[$cmid] : [];
    ...
    public function GetTotalDocumentCount()
    {
        ...
        } // fetch one document and determine the count
        ...
        if ($this->folders===null) {
            if ($populate_cache) {
                ...
                foreach ($this->folders as $child_folder) {
                    if ($path_segment===$child_folder->path_segment) {
                        return $child_folder;
                    }
                }
            } else {
                ...
                if (($child_folder=$this->repository->TryGetSubFolderNodeByPathSegment($this->node_id, $path_segment, true))!==null) {
                    return $child_folder;
                }
            }
        } else {
    ...
    public function CreateDocument($name, $data, $size=-1)
    {
        ...
        if ($this->repository->GetRootTreeId()!=1) {
            ...
            $tmp = $this->repository->GetServer()->MediaUpdate($new_media_id, [], $categories);
            if ($tmp===false) {
                return false;
            }
        }
    ...
    public function MoveToPath($trg_path, $new_name=null, $overwrite_mode=null)
    {
        ...
        $new_parent_cat_id = $new_parent_node->GetFolderId();
        if (($old_parent_node=$this->GetParentNode())!==null) {
            $old_parent_node->InvalidateChildCaches();
        }
...
class MjCRepository
{
    ...
    {
        if (isset($this->folders_by_path_cache['/'])) {
    ...
    {
        if (strncmp($node_id, 'd_', 2)===0 /*$this->IsDocumentNodeId($node_id)*/) {
            return $this->GetDocumentNodeById($node_id);
        }
        if (strncmp($node_id, 'f_', 2)===0 /*$this->IsFolderNodeId($node_id)*/) {
            return $this->GetFolderNodeById($node_id);
        }
    ...
    public function GetDocumentNodeById($node_id)
    {
        ...
        $media_info = $this->server->MediaInfo([$media_id], $this->GetDocumentNodeRequiredMetaIds());
        if (count($media_info['meta'])===0 && count($media_info['categories'])===0) {
            throw new MjCObjectNotFoundException('object not found(A): node_id=' . $node_id);
        }
        $meta = isset($media_info['meta'][$media_id]) ? $media_info['meta'][$media_id] : [];
        $cats = isset($media_info['categories'][$media_id]) ? $media_info['categories'][$media_id] : [];
        $parent_node_id = null;
        if (isset($cats[$this->root_tree_id])) {
            foreach ($cats[$this->root_tree_id] as $cat_id) {
                if (!isset($this->categories[$cat_id])) {
                    $this->_load_category($cat_id);
                }
                if (isset($this->categories[$cat_id])) {
    ...
    public function GetFolderNodeById($node_id)
    {
        ...
        if (isset($this->folders_by_id_cache[$folder_id])) {
            return $this->folders_by_id_cache[$folder_id];
        }
        if (!isset($this->categories[$folder_id])) {
            $this->_load_category($folder_id);
            if (!isset($this->categories[$folder_id])) {
                throw new MjCObjectNotFoundException('object not found(B): node_id=' . $node_id);
            }
        }
     ...
     * @param string $node_id
     * @param bool $skip_parent_validation
     * @return MjCFolder[]
     */
    public function GetSubFolderNodesById($node_id, $skip_parent_validation=false)
    {
        ...
        if ($skip_parent_validation===false) {
            if (!isset($this->categories[$folder_id])) {
                $this->_load_category($folder_id);
                if (!isset($this->categories[$folder_id])) {
                    throw new MjCObjectNotFoundException('object not found(C): node_id=' . $node_id);
                }
            }
        }
        $subfolders = [];
        foreach ($this->_load_category_children($folder_id) as $cand_cat_id) {
            if (isset($this->folders_by_id_cache[$cand_cat_id])) {
                $subfolders[] = $this->folders_by_id_cache[$cand_cat_id];
            } else {
                $subfolders[] = $this->CreateFolderNode2($cand_cat_id, $this->categories[$cand_cat_id]);
            }
        }
        return $subfolders;
    }
    ...
    public function TryGetSubFolderNodeByPathSegment($node_id, $path_segment, $skip_parent_validation=false)
    {
        ...
        if ($skip_parent_validation===false) {
            if (!isset($this->categories[$folder_id])) {
                $this->_load_category($folder_id);
                if (!isset($this->categories[$folder_id])) {
                    throw new MjCObjectNotFoundException('object not found(D): node_id=' . $node_id);
                }
            }
        }
        $subfolders = [];
        foreach ($this->_load_category_children($folder_id) as $cand_cat_id) {
            ...
            if ($path_segment===$cand_cmeta['filename']) {
                if (isset($this->folders_by_id_cache[$cand_cat_id])) {
                    return $this->folders_by_id_cache[$cand_cat_id];
                }
                return $this->CreateFolderNode2($cand_cat_id, $cand_cmeta);
            }
        }
        return null;
    }
    ...
    {
        if (!($path instanceof MjCPath)) {
            $path = new MjCPath($path);
        }
    ...
    public function CreateDocumentNodeId($doc_id)
    {
        return 'd_' . $doc_id;
    }
    public function CreateFolderNodeId($folder_id)
    {
        return 'f_' . $folder_id;
    }
    public function IsDocumentNodeId($node_id)
    {
        return strncmp($node_id, 'd_', 2)===0;
    }
    public function IsFolderNodeId($node_id)
    {
        return strncmp($node_id, 'f_', 2)===0;
    }
    public function CreateDocumentIdFromNodeId($node_id)
    {
        return substr($node_id, 2);
    }
    public function CreateFolderIdFromNodeId($node_id)
    {
        return substr($node_id, 2);
    }
    ...
    {
        if ($filename===null) {
            $filename = isset($meta[1]) ? mj_make_filename($meta[1][0]) : null;
        }
        $attrs = [	'node_id'			=> 'd_' . $media_id,  							// $this->CreateDocumentNodeId($media_id)
    ...
    public function CreateFolderNode2($cat_id, $cmeta)
    {
        ...
        $attrs = [	'node_id'			=> 'f_' . $cat_id,								// $this->CreateFolderNodeId($cat_id)
                        'folder_id'			=> $cat_id,
                        'parent_node_id'	=> $parent_node_id,
