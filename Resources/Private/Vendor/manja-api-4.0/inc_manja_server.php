...
{
    return isset($string[0]) ? explode($delimiter, $string) : [];
}
...
class ManjaServer
{
    ...
    private $server_version;				// string "minor.major", e.g. "2.3"
    ...
    private $connected_username;
    private $connected_user_password;
    private $connected_session_id;
    ...
    public function Connect()
    {
        ...
        if (isset($this->server_features['ssl'])) {
            ...
            if (!isset($this->server_features['plain'])) {
    ...
    public function SSL($ssl_ctx_opts=null)
    {
        ...
        */
        ...
        if (!isset($ssl_ctx_opts['verify_peer'])) {
            $ssl_ctx_opts['verify_peer'] = false;
        }
        if (count($ssl_ctx_opts)>0) {
            stream_context_set_option($this->ctx, ['ssl'=>$ssl_ctx_opts]);
        }
        ...
        if (defined('STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT')) {
            $crypto_types = $crypto_types|STREAM_CRYPTO_METHOD_TLSv1_1_CLIENT|STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT;
        }
    ...
    {
        // login
        $par = ['user'=>$username, 'pass'=>$password, 'client_id'=>$this->cfg_client_id];
        if ($remote_host!==null) {
            $par['remote_host'] = $remote_host;
        }
        if ($remote_port!==null) {
            $par['remote_port'] = $remote_port;
        }
    ...
    {
        $par = ['session_id'=>$sid, 'client_id'=>$this->cfg_client_id];
        if ($get_data) {
            $par['get_data'] = '1';
        }
        if ($get_roles) {
    ...
    {
        if ($keys === null) {
            return $this->_cmd('session get');
        }
        return $this->_cmd('session get', ['parameters'=>implode(',', $keys)]);
    }
    ...
    public function LicenseGet()
    {
        ...
        if (is_array($tmp) && isset($tmp['license'])) {
            return $tmp['license'];
        }
        return false;
    }
    ...
    public function UtilNowGet($utc=true, $interval1=null, $interval2=null)
    {
        $par = [ 'utc' => $utc?'1':'0' ];
        if ($interval1!==null) {
            $par['interval1'] = $interval1;
        }
        if ($interval2!==null) {
            $par['interval2'] = $interval2;
        }
    ...
    {
        $par = [ 'fulltext'=>$fulltext, 'fulltext_match_partial'=>$fulltext_match_partial?'1':'0', 'fulltext_match_category_titles'=>$in_category_titles?'1':'0', 'result_list_id'=>$result_list_id, 'work_list_id'=>$work_list_id ];
        if (is_array($meta_ids)) {
            $par['meta_ids'] = implode(',', $meta_ids);
        } elseif (is_string($meta_ids)) {
            $par['meta_ids'] = $meta_ids;
        }
        ...
        if ($filters!==null) {
            $par = $filters+$par;
        }		//array_merge( $par, $filters );
        if ($similar_to_media_ids!==null) {
            $par['similar_to_media_ids'] = implode(',', $similar_to_media_ids);
        }
        return $this->_cmd('index search', $par);
    }

    public function IndexSearch2($fulltext, $fulltext_match_partial=false, $result_list_id, $work_list_id, $meta_ids, $dominant_color, $dominant_color_limit, $filters, $similar_to_media_ids, $in_categories, $in_trees, $not_in_trees, $conjunct_mode_trees=[], $in_category_titles=false)
    {
        $par = [ 'fulltext'=>$fulltext, 'fulltext_match_partial'=>$fulltext_match_partial?'1':'0', 'fulltext_match_category_titles'=>$in_category_titles?'1':'0', 'result_list_id'=>$result_list_id, 'work_list_id'=>$work_list_id,
            'filter_categories'=>'1',
    ...
    public function IndexListFilter($result_list_id, $work_list_id, $in_categories, $in_trees, $not_in_trees, $conjunct_mode_trees=[])
    {
        $par = [ 'result_list_id'=>$result_list_id, 'work_list_id'=>$work_list_id, 'in_categories'=>implode(',', $in_categories), 'in_trees'=>implode(',', $in_trees), 'not_in_trees'=>implode(',', $not_in_trees), 'conjunct_mode_trees'=>implode(',', $conjunct_mode_trees) ];
        return $this->_cmd('index list filter', $par);
    }

    public function IndexListCounts($list_id)
    {
        $tmp = $this->_cmd('index list counts', ['list_id'=>$list_id]);
        if ($tmp===false) {
            return false;
        }
        ...
        foreach ($tmp as $k=>$v) {
            ...
            } elseif (strncmp($k, 'unassigned.', 11)===0) {
                $unassigned_to_trees[substr($k, 11)] = x_explode(',', $v);
            }
        }
        return [ 'assigned'=>$assigned_to_categories, 'unassigned'=>$unassigned_to_trees ];
    }
    ...
    {
        $par = [ 'list_id'=>$list_id, 'offset'=>$offset, 'count'=>$count ];
        if ($filter_deleted) {
            if (is_array($meta_ids)) {
                $meta_ids[] = -1;
            } else {
        ...
        if (is_array($meta_ids) && isset($meta_ids[0])) {
            $par['meta_ids'] = implode(',', array_keys(array_flip($meta_ids)));
        }
        if ($with_categories) {
            $par['with_categories'] = '1';
        }
        if ($with_relevance) {
            $par['with_relevance'] = '1';
        }
        ...
        if (($rc=(int)$tmp['result_count']) > 0) {
            $idx = $offset;
            foreach (x_explode(',', $tmp['result']) as $id) {
                $mids[$idx++] = $id;
            }
            if ($with_relevance) {
                $idx = $offset;
                foreach (x_explode(',', $tmp['relevance']) as $val) {
                    $relevance[$idx++] = $val;
                }
            }
        }
    ...
    {
        $tmp = $this->_cmd('index meta suggestions get', [ 'meta_id'=>$meta_id, 'cur_count'=>$cur_count, 'freq_count'=>$freq_count, 'char_limit'=>$char_limit ]);
        if ($tmp===false) {
            return false;
        }
    ...
    public function IndexSearchSuggestionsGet($meta_ids, $count, $char_limit, $pattern, $in_category_titles=false)
    {
        ...
        $par = [ 'meta_ids'=>$meta_ids, 'fulltext_match_category_titles'=>$in_category_titles?'1':'0', 'count'=>$count, 'char_limit'=>$char_limit, 'pattern'=>$pattern ];
    ...
    {
        $tmp = $this->_cmd('index meta values get', ['meta_id'=>$meta_id]);
        if ($tmp===false) {
            return false;
        }
        return $this->_parse_numeric_result($tmp);
    ...
    public function MediumGet($media_id, $stream=true, $filename=null, $options=[])
    {
        ...
        if ($stream && isset($tmp['payload'])) {
            $this->_stream_payload($tmp, $filename, $options);
        }
        return $tmp;
    }

    public function MediumDownload($media_id, $dlf_id, $stream=true, $filename=null, $options=[])
    {
        $options['media_id'] = $media_id;
    ...
    {
        $options = ['media_id'=>$media_id, 'dlf_id'=>$dlf_id];
    ...
    {
        $opts = ['media_id'=>$media_id, 'page'=>$page];
        if ($box!==null) {
            $opts['box'] = $box;
        }
    ...
    public function MediumUpload($file, $media_id=null, $cat_id_tree_1=1, $filename='', $new_media_id=null)
    {
        ...
        if ($media_id===null) {
            ...
            if ($new_media_id!==null) {
                $opts['new_media_id'] = $new_media_id;
            }
        } else {
    ...
    public function MediumXmpGet($media_id, $stream=true, $filename=null, $options=[])
    {
        $options['media_id'] = $media_id;
        if ($stream) {
            $options['exit']='1';
    ...
    {
        $opts = [ 'media_id'=>$media_id, 'persist_as_sidecar_file'=>$persist_as_sidecar_file?'1':'0', 'do_not_create_version'=>$do_not_create_version?'1':'0' ];
        $opts['payload'] = filesize($file);
        $fp = fopen($file, 'rb');
        if ($fp === false) {
            $this->error_code = 'fs/1';
    ...
    {
        $filename = Normalizer::normalize($filename);
        $opts = [ 'media_id'=>$media_id, 'cp_id'=>$cp_id, 'filename'=>$filename ];
        $opts['payload'] = filesize($file);
        $fp = fopen($file, 'rb');
        if ($fp === false) {
            $this->error_code = 'fs/1';
    ...
    {
        return $this->_cmd('medium custom preview remove', [ 'media_id'=>$media_id, 'cp_id'=>$cp_id ]);
    }
    ...
    public function MediumCustomPreviewGet($options, $stream=true, $filename=null)
    {
        ...
        if ($stream && isset($tmp['payload'])) {
            $this->_stream_payload($tmp, $filename, $options, 60);
        }
        return $tmp;
    }

    public function MediumXFDFInfo($media_id, $version=0, $with_xfdf=false)
    {
        $opts = [ 'media_id'=>$media_id ];
        if ($version>0) {
    ...
    public function MediumXFDFGet($media_id, $version=0, $if_modified_since=null, $if_none_match=null, $set_empty_to_default_xfdf=false)
    {
        $opts = [ 'media_id'=>$media_id ];
        if ($version>0) {
            $opts['version'] = $version;
        }
        if ($if_modified_since!==null && $if_modified_since>0) {
            $opts['if_modified_since'] = $if_modified_since;
        }
        if ($if_none_match!==null) {
            if ($if_none_match===true) {
                $opts['if_none_match'] = '*';
            } else {
                $opts['if_none_match'] = implode(',', $if_none_match);
            }
        }
        if ($set_empty_to_default_xfdf) {
            $opts['set_empty_to_default_xfdf'] = '1';
        }
    ...
    {
        $opts = [ 'media_id'=>$media_id, 'xfdf'=>$xfdf ];
        if ($update_embedded) {
            $opts['update_embedded'] = '1';
        }
    ...
    {
        $opts = [ 'media_id'=>$media_id, 'action'=>$action, 'annotation_id'=>$annotation_id, 'parent_author_id'=>$parent_author_id, 'xfdf'=>$xfdf ];
        if ($update_embedded) {
            $opts['update_embedded'] = '1';
        }
    ...
    public function MediumClone($from_media_id, $to_media_id=null)
    {//, $delete_from=false ) {
        $opts = [ 'from_media_id'=>$from_media_id ];
        if ($to_media_id!==null) {
            $opts['to_media_id'] = $to_media_id;
        }
    ...
    public function MediumStructureInfo($media_id, $page=0, $box=null, $options=[])
    {
        $opts = $options;
        $opts['media_id'] = $media_id;
        if ($page>0) {
        ...
        foreach ($tmp as $k=>$v) {
            ...
            if ($kp[0]==='p') {
                $pageno = $kp[1];
                if (!isset($result[$pageno])) {
                    $result[$pageno] = [ 'page'=>$pageno, 'media_box'=>'', 'ref_box'=>'', 'elements'=>[] ];
                }
                switch ($kp[2]) {
                    case 'ref_box':
    ...
    public function MediaStructureCompare($media_id_a, $media_id_b, $page_a, $page_b, $box_a, $box_b, $options=[])
    {
        ...
        $opts = ['media_id_a'=>$media_id_a, 'media_id_b'=>$media_id_b, 'page_a'=>$page_a, 'page_b'=>$page_b, 'box_a'=>$box_a, 'box_b'=>$box_b] + $options;
        ...
        $result = [
            ...
            'document_a'		=> [ 'npages'=>$tmp['dd.a.npages'] ],
            'document_b'		=> [ 'npages'=>$tmp['dd.b.npages'] ],
        ];
        $page_diffs = [];
        foreach ($tmp as $k=>$v) {
            $kp = explode('.', $k, 5); // key parts
            if ($kp[0]==='pd') { // pd.
                ...
                if (!isset($page_diffs[$pd_key])) {
                    $page_diffs[$pd_key] = ['elem_diffs'=>[]];
                }
                switch ($kp[2]) {
                    ...
                    case 'ndifferences':
                        ...
                        break;
                    case 'a':
                        if ($kp[3]==='media_box' || $kp[3]==='ref_box') {
                            $page_diffs[$pd_key]['info_a'][$kp[3]] = x_explode2floats(' ', $v);
                        } else {
                            $page_diffs[$pd_key]['info_a'][$kp[3]] = $v;
                        }
                        break;
                    case 'b':
                        if ($kp[3]==='media_box' || $kp[3]==='ref_box') {
                            $page_diffs[$pd_key]['info_b'][$kp[3]] = x_explode2floats(' ', $v);
                        } else {
                            $page_diffs[$pd_key]['info_b'][$kp[3]] = $v;
                        }
                        break;
                    case 'ed':
                        $ed_idx = (int)$kp[3];
                        if ($kp[4]==='bbox_a' || $kp[4]==='bbox_b') {
                            $page_diffs[$pd_key]['elem_diffs'][$ed_idx][$kp[4]] = x_explode2floats(' ', $v);
                        } else {
                            $page_diffs[$pd_key]['elem_diffs'][$ed_idx][$kp[4]] = $v;
                        }
                        break;
                }
            }
    ...
    public function MediaInfo($media_ids, $meta_ids=null)
    {
        ...
        foreach ($tmp as $k=>$v) {
            ...
            if ($k[0]==='m') {
                ...
                $c[$kp[1]][$kp[2]] = x_explode(',', $v);
            } elseif ($k[0]==='v') {
                $vn[$kp[1]] = $v;
            }
        }
        $this->_sort_metadata_values($m);
        return ['meta'=>$m, 'categories'=>$c, 'versions'=>$vn];
    }
    ...
    public function MediaCategoriesList($media_ids)
    {
        ...
        $tmp = $this->_cmd('media categories list', ['media_ids'=>implode(',', $media_ids)]);
        if ($tmp===false) {
            return false;
        }
        $c = [];
    ...
    public function MediaUpdate($media_ids, $meta, $categories, $do_not_create_version=false)
    {
        ...
        $p = [ 'media_ids'=>implode(',', $media_ids) ];
        if ($do_not_create_version) {
            $p['do_not_create_version'] = '1';
        }
    ...
    public function MediaStatus($media_ids)
    {
        ...
        $tmp = $this->_cmd('media status', ['media_ids'=>implode(',', $media_ids)]);
        if ($tmp===false) {
            return false;
        }
    ...
    public function MediaDelete($media_ids, $rebuild_temp_file_references=false, $defered=false)
    {
        ...
        $p = ['media_ids'=>implode(',', $media_ids)];
        if ($rebuild_temp_file_references) {
            $p['rebuild_temp_file_references'] = '1';
        }
        if ($defered) {
            $p['defered'] = '1';
        }
    ...
    {
        return $this->_cmd('meta def move', ['meta_id'=>$meta_id, 'after_id'=>$after_id]);
    }
    ...
    {
        $p = [ 'meta_id'=>$meta_id, 'value'=>$value, 'op'=>$op ];
        $tmp = $this->_cmd('meta search', $p);
        if ($tmp===false) {
            return false;
        }
        if (!isset($tmp['media_ids']) || !isset($tmp['media_ids'][0])) {
            return [];
        }
    ...
    public function UserdataSet($user_id=0, $key_value_pairs=[])
    {
        $p = $key_value_pairs;
        if ($user_id) {
            $p['user_id'] = $user_id;
    ...
    public function ClientdataSet($key_value_pairs=[], $client_id=null, $lock_keys=false)
    {
        if ($client_id===null) {
            $client_id = $this->cfg_client_id;
        }
    ...
    public function ClientdataRemove($parameters, $client_id=null)
    {
        ...
        return $this->_cmd('clientdata remove', ['client_id'=>$client_id, 'parameters'=>implode(',', $parameters)]);
    }

    public function RightsMediaGet($media_ids, $action, $bool_result=false)
    {
        $tmp = $this->_cmd('rights media get', ['media_ids'=>implode(',', $media_ids), 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        $tmp = $this->_parse_dot_syntax($tmp);
        if ($bool_result) {
            foreach ($tmp as $mid=>&$inf) {
                $inf = $inf['allowed']==='1';
            }
        }
        return $tmp;
    ...
    {
        $tmp = $this->_cmd('rights media get', ['media_ids'=>$media_id, 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        $k = 'allowed.' . $media_id;
    ...
    {
        $tmp = $this->_cmd('user rights media get', ['user_id'=>$user_id, 'media_ids'=>implode(',', $media_ids), 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        $tmp = $this->_parse_dot_syntax($tmp);
    ...
    {
        $tmp = $this->_cmd('group rights media get', ['group_id'=>$group_id, 'media_ids'=>implode(',', $media_ids), 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        $tmp = $this->_parse_dot_syntax($tmp);
    ...
    {
        $tmp = $this->_cmd('rights categories get', ['cat_ids'=>implode(',', $cat_ids), 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        $tmp = $this->_parse_dot_syntax($tmp);
        if ($bool_result) {
            foreach ($tmp as $cid=>&$inf) {
                $inf = $inf['allowed']==='1';
            }
        }
        return $tmp;
    ...
    {
        $tmp = $this->_cmd('rights list get', ['list_id'=>$list_id, 'offset'=>$offset, 'count'=>$count, 'action'=>$action]);
        if ($tmp===false) {
            return false;
        }
        if (isset($tmp['allowed'])) {
            $tmp['allowed'] = x_explode(',', $tmp['allowed']);
        }
        return $tmp;
    }
    public function RightsListGet2($list_id, $offset, $count, $actions)
    {
        $tmp = $this->_cmd('rights list get 2', ['list_id'=>$list_id, 'offset'=>$offset, 'count'=>$count, 'actions'=>implode(',', $actions)]);
        if ($tmp===false) {
            return false;
        }
        $r = [];
    ...
    {
        $tmp = $this->_cmd('rights tree get', ['action'=>$action]);
        if ($tmp===false) {
            return false;
        }
    ...
    public function ArchiveCreate($media_items, $lifetime, $intent='')
    {
        ...
        $options = [	'media_ids'  => implode(',', $media_ids),
    ...
    {
        return $this->_cmd('archive ready', ['archive_id'=>$archive_id]);
    }
    ...
    {
        $tmp = $this->_cmd('download format list', ['options'=>$with_options?'1':'']);
        if ($tmp===false) {
            return false;
        }
        $r = [];
        $ktrans = [ 'mc'=>'media_class', 'so'=>'sort', 'aa'=>'acl_action', 'fo'=>'format', 'na'=>'name', 'fn'=>'filename' ];
        foreach ($tmp as $k=>$v) {
            $ka = explode('.', $k, 3);										// xx.<dlf_id> or o.<dlf_id>.<opt_name>
            if (isset($ka[1])) {
                $dlf_id = $ka[1];
                if ($ka[0]==='o') {
                    $r[$dlf_id]['options'][$ka[2]] = $v;
                }		// o.<dlf_id>.<opt_name>
                else {
                    $r[$dlf_id][$ktrans[$ka[0]]] = $v;
                }						// xx.<dlf_id>
            }
        }
        foreach ($r as $k=>&$v) {
            ...
            $v['id'] = $k;
            // add empty options array, if it does not exist yet
            if ($with_options && !isset($v['options'])) {
                $v['options'] = [];
            }
        }
        unset($v);
        if ($filter_media_class!==null) {
            $r2 = [];
            foreach ($r as $k=>&$v) {
                if ($v['media_class']==$filter_media_class) {
                    $r2[$k] = $v;
                }
            }
            //unset($v);
    ...
    {
        $tmp = $this->_cmd('download format get', ['dlf_id'=>$dlf_id, 'options'=>$with_options?'1':'']);
        if ($tmp===false) {
            return false;
        }
        $r = [];
    ...
    {
        return $this->_cmd('download format delete', ['dlf_id'=>$dlf_id]);
    }
    ...
    {
        $tmp = $this->_cmd('preview format list', ['options'=>$with_options?'1':'']);
        if ($tmp===false) {
            return false;
        }
        $r = [];
        $ktrans = [ 'mc'=>'media_class', 'so'=>'sort', 'fo'=>'format', 'na'=>'name' ];
        foreach ($tmp as $k=>$v) {
            $ka = explode('.', $k, 3);
            if (isset($ka[1])) {
                $pf_id = $ka[1];
                if ($ka[0]==='o') {
                    $r[$pf_id]['options'][$ka[2]] = $v;
                } else {
                    $r[$pf_id][isset($ktrans[$ka[0]])?$ktrans[$ka[0]]:$ka[0]] = $v;
                }
            }
        }
        foreach ($r as $k=>&$v) {
    ...
    {
        $tmp = $this->_cmd('preview format get', ['pf_id'=>$pf_id, 'options'=>$with_options?'1':'']);
        if ($tmp===false) {
            return false;
        }
        $r = [];
    ...
    {
        return $this->_cmd('preview format delete', ['pf_id'=>$pf_id]);
    }

    public function MediumPreviewFormatList($media_id, $with_options=false, $filter_media_class=null)
    {
        $par = ['media_id'=>$media_id, 'options'=>$with_options?'1':''];
        if ($filter_media_class!==null) {
            $par['flt_media_class'] = $filter_media_class;
        }
        $tmp = $this->_cmd('medium preview format list', $par);
        if ($tmp===false) {
            return false;
        }
        $ktrans = [	'mc'=>'media_class', 'so'=>'sort', 'fs'=>'file_size', 'fo'=>'format', 'na'=>'name',
    ...
    public function MediaPreviewFormatList($media_ids, $with_options=false, $filter_media_class=null)
    {
        ...
        // mj 2.4
        $par = ['media_ids'=>implode(',', $media_ids), 'options'=>$with_options?'1':''];
        if ($filter_media_class!==null) {
            $par['flt_media_class'] = $filter_media_class;
        }
    ...
    {
        $tmp = $this->_cmd('medium preview progress get', ['media_id'=>$media_id]);
        if ($tmp===false) {
            return false;
        }
        return $this->_parse_dot_syntax($tmp);
    ...
    {
        return $this->_cmd('medium prioritize processing', ['media_id'=>$media_id]);
    }

    public function FolderCategoryList($cat_id, $compact_result=false)
    {
        $tmp = $this->_cmd('folder category list', ['cat_id'=>$cat_id, 'compact_format'=>'1']);
        if ($tmp===false) {
            return false;
        }
    ...
    {
        $tmp = $this->_cmd('folder get', ['cat_id'=>$cat_id, 'compact_format'=>'1']);
        if ($tmp===false) {
            return false;
        }
    ...
    {
        $par = [ 'cat_id'=>$cat_id, 'offset'=>$offset, 'count'=>$count ];
        if (is_array($meta_ids) && count($meta_ids)) {
            $par['meta_ids'] = implode(',', $meta_ids);
        }
        ...
        if ((int)$tmp['result_count'] > 0) {
            $idx = $offset;
            foreach (explode(',', $tmp['result']) as $id) {
                $mids[$idx++] = $id;
            }
        }
        // meta-data
        ...
        $this->_sort_metadata_values($m);
        return [	'result_count' => $tmp['result_count'],
            'media_ids' => $mids,
            'meta' => $m ];
    }
    ...
    {
        $par = [ 'cat_id'=>$cat_id, 'filename'=>$filename ];
        if (is_array($meta_ids) && count($meta_ids)) {
            $par['meta_ids'] = implode(',', $meta_ids);
        }
    ...
    public function ExportQueueAddMedia($channel_id, $media_ids, $formats, $filenames)
    {
        ...
        return $this->_cmd('export queue add media', ['channel_id'=>$channel_id, 'media_ids'=>$media_ids, 'formats'=>$formats, 'filenames'=>$filenames]);
    }
    ...
    {
        $tmp = $this->_cmd('version medium list versions', ['media_id'=>$media_id]);
        if ($tmp===false) {
            return false;
        }
        if (isset($tmp['versions'])) {
            $tmp['versions'] = x_explode(',', $tmp['versions']);
        }
        return $tmp;
    }

    public function VersionMediumGet($media_id, $version, $stream=true, $filename=null, $options=[])
    {
        $options['media_id'] = $media_id;
    ...
    public function VersionMediumXmpGet($media_id, $version, $stream=true, $filename=null, $options=[])
    {
        $options['media_id'] = $media_id;
        $options['version'] = $version;
        if ($stream) {
    ...
    {
        $p = ['media_id'=>$media_id, 'version'=>$version];
        if (is_array($meta_ids) && count($meta_ids)) {
            $p['meta_ids'] = implode(',', $meta_ids);
        } elseif ($meta_ids!==null) {
            $p['meta_ids'] = $meta_ids;
        ...
        return ['meta'=>$m, 'categories'=>$c, 'is_latest_version'=>isset($tmp['is_latest_version'])&&$tmp['is_latest_version']==='1'];
    }
    ...
    {
        $para = [ 'media_id'=>$media_id, 'version'=>$version ];
        if ($comment!==null) {
            $para['version_comment'] = $comment;
        }
    ...
    {
        $opts = ['media_id'=>$media_id, 'version'=>$version, 'page'=>$page];
        if ($box!==null) {
            $opts['box'] = $box;
        }
    ...
    public function VersionMediaStructureCompare($media_id_a, $media_id_b, $version_a, $version_b, $page_a, $page_b, $box_a, $box_b, $options=[])
    {
        ...
        $opts = ['media_id_a'=>$media_id_a, 'media_id_b'=>$media_id_b, 'version_a'=>$version_a, 'version_b'=>$version_b, 'page_a'=>$page_a, 'page_b'=>$page_b, 'box_a'=>$box_a, 'box_b'=>$box_b] + $options;
    ...
    {
        return $this->_cmd('lock acquire', ['media_id'=>$media_id]);
    }

    public function LockRelease($media_id)
    {
        return $this->_cmd('lock release', ['media_id'=>$media_id]) === false ? false : true;
    }

    public function LockGet($media_id)
    {
        return $this->_cmd('lock get', ['media_id'=>$media_id]);
    }
    ...
    {
        $roles = implode(',', $roles);
        return $this->_cmd('group add', ['name'=>$name, 'roles'=>$roles]);
    }
    public function GroupDelete($group_id)
    {
        return $this->_cmd('group delete', ['group_id'=>$group_id]);
    }
    public function GroupUpdate($group_id, $name, $roles)
    {
        $roles = implode(',', $roles);
        return $this->_cmd('group update', ['group_id'=>$group_id, 'name'=>$name, 'roles'=>$roles]);
    }
    ...
    public function UserList($userdata=null, $with_roles=false)
    {
        ...
        if ($userdata!==null && $userdata!==false) {
            $opts['userdata'] = $userdata===true ? '1' : implode(',', $userdata);
        }
        if ($with_roles===true) {
            $opts['with_roles'] = '1';
        }
    ...
    {
        $opts = ['fulltext'=>$fulltext];
        if ($match_partial===true) {
            $opts['match_partial'] = '1';
        }
        if ($userdata!==null && $userdata!==false) {
            $opts['userdata'] = $userdata===true ? '1' : implode(',', $userdata);
        }
    ...
    {
        return $this->_cmd('user exists', ['userdata_name'=>$userdata_name, 'userdata_value'=>$userdata_value]);
    }
    public function UserAdd($login, $pass, $expires, $groups, $pass_is_md5=false, $active=true)
    {
        $opts = ['login'=>$login, 'pass'=>$pass, 'expires'=>$expires, 'active'=>$active?'1':'0', 'groups'=>implode(',', $groups)];
        if ($pass_is_md5) {
            $opts['pass_is_md5'] = '1';
        }
    ...
    {
        return $this->_cmd('user update', ['user_id'=>$user_id, 'login'=>$login, 'expires'=>$expires, 'active'=>$active?'1':'0', 'groups'=>implode(',', $groups)]);
    }
    public function UserPassword($user_id, $pass)
    {
        return $this->_cmd('user password', ['user_id'=>$user_id, 'pass'=>$pass]);
    }
    public function UserLoginCheck($login, $pass)
    {
        $tmp = $this->_cmd('user login check', ['user'=>$login, 'pass'=>$pass]);
        if ($tmp===false) {
            return false;
        }
        return $tmp['user_id'];
    ...
    {
        return $this->_cmd('user session create', ['user_id'=>$user_id]);
    }
    public function UserSessionCreateByLogin($login)
    {
        return $this->_cmd('user session create', ['login'=>$login]);
    }
    ...
    public function ACLItemList($acl_id=null, $cat_id=null, $media_id=null)
    {
        ...
        foreach ($acl_items as $k=>&$v) {
            $v['order'] = (int)$v['order'];
        }
        //unset($v);
        uasort($acl_items, '_mj_cmp_acl_items');
        return ['acl_id'=>$acl_id, 'acl_items'=>$acl_items];
    }
    public function ACLItemAdd($acl_id, $cat_id, $media_id, $apply_to_group_id, $apply_to_user_id, $allow_deny, $action)
    {
        ...
        if ($apply_to_group_id) {
            $p['group_id'] = $apply_to_group_id;
        } elseif ($apply_to_user_id) {
            $p['user_id'] = $apply_to_user_id;
        }
    ...
    {
        return $this->_cmd('acl item delete', ['acl_item_id'=>$acl_item_id]);
    }
    public function ACLItemMove($acl_item_id, $after_id, $target_acl_id)
    {
        return $this->_cmd('acl item move', ['acl_item_id'=>$acl_item_id, 'after_id'=>$after_id, 'target_acl_id'=>$target_acl_id]);
    }
    ...
    {
        return $this->_cmd('acl copy', ['from_media_id'=>$from_media_id, 'to_media_ids'=>implode(',', $to_media_ids)]);
    }

    public function TreeAdd($name, $visibility, $multifiling, $conjunct_mode=false)
    {
        return $this->_cmd('tree add', ['name'=>$name, 'visibility'=>$visibility, 'multifiling'=>$multifiling, 'conjunct_mode'=>$conjunct_mode]);
    }
    public function TreeDelete($tree_id)
    {
        return $this->_cmd('tree delete', ['tree_id'=>$tree_id]);
    }
    public function TreeUpdate($tree_id, $name, $visibility, $multifiling, $conjunct_mode=false)
    {
        return $this->_cmd('tree update', ['tree_id'=>$tree_id, 'name'=>$name, 'visibility'=>$visibility, 'multifiling'=>$multifiling, 'conjunct_mode'=>$conjunct_mode]);
    }
    ...
    {
        $p = ['name'=>$name, 'tree_id'=>$tree_id, 'parent_id'=>$parent_id, 'after_id'=>$after_id];
    ...
    {
        $p = ['cat_id'=>$cat_id, 'parent_id'=>$parent_id, 'after_id'=>$after_id];
        // note: $after_id=0 -> sort as first node in parent, $after_id=-1 -> sort as last node in parent, every other $after_id must be a valid category id
        if ($sort_alphabetically_into_parent) {
            $p['insert_alphabetically_sorted'] = '1';
        }
    ...
    {
        return $this->_cmd('category sort', ['cat_id'=>$cat_id, 'recursive'=>$recursive?'1':'0']);
    }
    public function CategoryList($tree_id, $dont_filter=false, $compact_result=false)
    {
        $p = ['tree_id'=>$tree_id, 'compact_format'=>'1'];
        if ($dont_filter) {
            $p['dont_filter'] = '1';
        }
    ...
    {
        $p = ['cat_id'=>$cat_id, 'compact_format'=>'1'];
    ...
    {
        return $this->_cmd('category custom preview from medium', [ 'cat_id'=>$cat_id, 'media_id'=>$media_id ]);
    }
    ...
    {
        return $this->_cmd('category custom preview remove', [ 'cat_id'=>$cat_id ]);
    }
    public function CategoryCustomPreviewInfo($cat_id)
    {
        return $this->_cmd('category custom preview info', [ 'cat_id'=>$cat_id ]);
    }
    ...
    {
        $para = ['offset'=>$offset, 'count'=>$count, 'tz'=>$tz];
        if ($filter!==null) {
            if (isset($filter['date']) && $filter['date']!=='') {
                $para['flt_date'] = $filter['date'];
            }
            if (isset($filter['date_from']) && $filter['date_from']!=='') {
                $para['flt_date_from'] = $filter['date_from'];
            }
            if (isset($filter['date_to']) && $filter['date_to']!=='') {
                $para['flt_date_to'] = $filter['date_to'];
            }
            if (isset($filter['entity_type']) && $filter['entity_type']!=='') {
                $para['flt_entity_type'] = $filter['entity_type'];
            }
            if (isset($filter['action']) && $filter['action']!=='') {
                $para['flt_action'] = $filter['action'];
            }
            if (isset($filter['user_type']) && $filter['user_type']!=='') {
                $para['flt_user_type'] = $filter['user_type'];
            }
            if (isset($filter['user_id']) && $filter['user_id']!=='') {
                $para['flt_user_id'] = $filter['user_id'];
            }
            if (isset($filter['client_id']) && $filter['client_id']!=='') {
                $para['flt_client_id'] = $filter['client_id'];
            }
            if (isset($filter['client_ids'])) {
                $para['flt_client_ids'] = is_array($filter['client_ids']) ? implode(',', $filter['client_ids']) : $filter['client_ids'];
            }
            if (isset($filter['fulltext_pattern']) && $filter['fulltext_pattern']!=='') {
                $para['flt_fulltext_pattern'] = $filter['fulltext_pattern'];
            }
            if (isset($filter['fulltext_match_partial']) && $filter['fulltext_match_partial']) {
                $para['flt_fulltext_match_partial'] = '1';
            }
        }
        ...
        $tmp = $this->_parse_dot_syntax($tmp);
        $ktrans = [ 'lid'=>'id', 'ltm'=>'ltime', 'uid'=>'user_id', 'unm'=>'user_name', 'sid'=>'session_id', 'act'=>'action', 'etp'=>'entity_type', 'ent'=>'entity', 'det'=>'details', 'cid'=>'client_id' ];
        $entries = [];
        foreach ($tmp as $e) {
            $o = [];
            foreach ($ktrans as $k1=>$k2) {
                $o[$k2] = trim($e[$k1]);
            }
            $entries[] = $o;
        }
    ...
    {
        $para = ['name'=>$name, 'target_url'=>$target_url, 'recursive_dirs'=>$recursive_dirs, 'recursive_dirs_tree_id'=>$recursive_dirs_tree_id, 'acl_action'=>$acl_action];
    ...
    {
        $para = ['channel_id'=>$channel_id, 'name'=>$name, 'target_url'=>$target_url, 'recursive_dirs'=>$recursive_dirs, 'recursive_dirs_tree_id'=>$recursive_dirs_tree_id, 'acl_action'=>$acl_action];
        return $this->_cmd('export channel update', $para);
    }
    public function ExportChannelDelete($channel_id)
    {
        return $this->_cmd('export channel delete', ['channel_id'=>$channel_id]);
    }

    public function ExportQueueAddAllMedia($channel_id, $filename_schema)
    {
        return $this->_cmd('export queue add all media', ['channel_id'=>$channel_id, 'filename_schema'=>$filename_schema]);
    }
    ...
    public function ExportLogGet($count=null, $channel_id=null, $geq_ltime=null, $leq_ltime=null, $since_log_id=null, $flt_success=null)
    {
        ...
        if ($geq_ltime!==null) {
            $para['geq_ltime'] = $geq_ltime;
        }
        if ($leq_ltime!==null) {
            $para['leq_ltime'] = $leq_ltime;
        }
        if ($since_log_id!==null) {
            $para['since_log_id'] = $since_log_id;
        }
        if ($flt_success!==null) {
            $para['flt_success'] = $flt_success;
        }
    ...
    public function WatermarkAdd($opts=[], $file=null)
    {
        $fp = null;
        if ($file!==null) {
            $opts['payload'] = filesize($file);
        ...
        if ($tmp===false || !isset($tmp['wm_id'])) {
            return false;
        }
        return $tmp['wm_id'];
    }

    public function WatermarkUpdate($wm_id, $opts=[], $file=null)
    {
        $fp = null;
        if ($file!==null) {
            $opts['payload'] = filesize($file);
    ...
    {
        return $this->_cmd('watermark delete', ['wm_id'=>$wm_id]);
    }

    public function WatermarkGet($wm_id)
    {
        return $this->_cmd('watermark get', ['wm_id'=>$wm_id]);
    }
    ...
    {
        return $this->_cmd('category watermark get', ['cat_id'=>$cat_id]);
    }

    public function CategoryWatermarkSet($cat_id, $wm_id, $wm_rule)
    {
        return $this->_cmd('category watermark set', ['cat_id'=>$cat_id, 'wm_id'=>$wm_id, 'wm_rule'=>$wm_rule]);
    }

    public function BulkPermissionsMediaInfo($media_ids, $actions, $group_id=0, $user_id=0)
    {
        $para = [ 'media_ids'=>implode(',', $media_ids), 'actions'=>implode(',', $actions) ];
        if ($group_id) {
    ...
    public function BulkPermissionsMediaApply($rules, $action, $group_id=0, $user_id=0)
    {
        ...
        foreach ($rules as $media_id=>$rule) {
            $para['r.' . $media_id] = $rule;
        }
    ...
    {
        $para = [ 'post_delayed'=>$post_delayed?'1':'0' ];
        if ($media_ids!==null) {
            $para['media_ids'] = implode(',', $media_ids);
        }
    ...
    public function get_payload($result, $reconnect=false)
    {
        ...
        if ($size!==-1) {
            ...
            if ($reconnect) {
                $this->Connect();
                if ($this->connected_ssl_active) {
                    $this->SSL($this->connected_ssl_ctx_opts);
                }
                if ($this->connected_session_id===null) {
                    $this->Login($this->connected_username, $this->connected_user_password);
                } else {
    ...
    private function _stream_payload($result, $filename=null, $options=[], $default_max_age_seconds=null)
    {
        ...
        if ($default_max_age_seconds===null) {
            $default_max_age_seconds = $payload_volatile ? 60*60 : 1*86400;
        }	// volatile -> 60min, nonvolatile -> 1d
        ...
        if (!function_exists('http_cache_control_headers')) {
            require_once __DIR__ . '/inc_util.php';
        }
        ...
        if ($filename!==null) {
            http_filename_headers($filename);
        }
        if ($size===-1) {
            ...
            foreach ($sresult as $k=>$v) {
                ...
                if (isset($t[1])) {
                    ...
                    if ($is_next_node) {
                        ...
                        if ($cur_n_2===$prev_n_2+1) {
                            $n[2] = -1;
                        } elseif ($cur_n_2===$prev_n_3+1) {
                            $n[2] = 0;
                        }
    ...
    {
        return [MC_IMAGE, MC_VIDEO, MC_AUDIO, MC_TEXT, MC_CONTAINER, MC_OTHER, MC_UNKNOWN];
    }
    ...
    {
        $filename_patterns = '';
        foreach ($download_formats as $df) {
            $filename_patterns .= ' ' . (isset($df['filename']) ? $df['filename'] : '');
        }
            ...
            foreach ($download_formats as $dlf_id => $dlf) {
                ...
                if ($fnp === '') {
                    ...
                    if ($fmt==='*') {
                        ...
                        if (!isset($mt2sfx_cache[$fmt])) {
                            $mt2sfx_cache[$fmt] = $this->GetSuffixFromMediaType($fmt);
                        }
                    ...
                    $lookup_meta_data_value = function ($meta_id) use (&$mmd,&$sfx) {
                        ...
                        $tmp = isset($mmd[$meta_id]) && count($mmd[$meta_id]) ? array_values($mmd[$meta_id]) : [''];
                        return trim($tmp[0]);
                    };
                    ...
                    $fn = preg_replace_callback('/%([-s0-9]+)\\.?([^%|]*)\\|?([-s0-9]*)\\.?([^%]*)%/', function ($match) use (&$lookup_meta_data_value,&$transform_meta_data_value) {
                        ...
                        if (isset($match[2][0])) {
                            $val = $transform_meta_data_value($val, $match[2]);
                        }
                        if ($val===null || $val==='') {
                            ...
                            if (isset($match[3][0])) {
                                ...
                                if (isset($match[4][0])) {
                                    $val = $transform_meta_data_value($val, $match[4]);
                                }
                            }
                        }
                        return $val;
    ...
    public function GetFilename($media_id, $dlf_id)
    {
        ...
        if (($df=$this->DownloadFormatGet($dlf_id, true))===false) {
            return null;
        }
        // get required meta_ids and meta_data
        $meta_ids = $this->GetFilenamesMetaIds([$df]);
        if (($meta_data=$this->MediaMetaList([$media_id], $meta_ids))===false) {
            return null;
        }
        // generate filename
    ...
    public function CreatePasswordResetRequest($user_id)
    {
        ...
        if (($tmp=$this->UserdataSet($user_id, ['reset-password-token'=>$token, 'reset-password-token-time'=>time()]))===false) {
            return false;
        }
        return $token;
    }
    ...
    {
        $ud = $this->UserdataGet($user_id, ['reset-password-token', 'reset-password-token-time']);
    ...
    {
        if (($tmp=$this->UserdataSet($user_id, ['reset-password-token'=>'', 'reset-password-token-time'=>'']))===false) {
            return false;
        }
        return true;
    }
    ...
    public function SanitizeUserdata(&$userdata)
    {
        ...
        foreach ($userdata as $k=>$v) {
            if (strncmp($k, 'sp2ud-', 6)===0) {
                unset($userdata[$k]);
            } // any "sp2ud-" data (=SessionParameters2UserData..)
        }
    }
    ...
    public function SanitizeUserlist(&$userlist)
    {
        foreach ($userlist as $k=>&$v) {
            if (isset($v['userdata'])) {
                $this->SanitizeUserdata($v['userdata']);
            }
        }
        //unset($v);
    }
    ...
    {
        $tmp = $this->_cmd('object get', ['type'=>$type, 'object_id'=>$object_id]);
        if ($tmp===false) {
            return false;
        }
    ...
    {
        $tmp = $this->_cmd('object list', [ 'type'=>$type, 'relation'=>$relation ]);
        if ($tmp===false) {
            return false;
        }
        ...
        foreach ($tmp as $id=>$content) {
            ...
            if (in_array('own', $listfilter) && $content['owner'] == $user_id) {
                $output[$id] = $tmp[$id];
            } elseif (in_array('relation', $listfilter) && $content['owner'] != $user_id && $content['is_public'] == 0) {
                $output[$id] = $tmp[$id];
            } elseif (in_array('public', $listfilter) && $content['is_public'] == 1) {
                $output[$id] = $tmp[$id];
            } elseif (in_array('', $listfilter)) {
                $output[$id] = $tmp[$id];
            }
        }
    ...
    {
        $tmp = $this->_cmd('object list', [ 'login'=>$login, 'type'=>$type, 'relation'=>$relation, 'style'=>$style ]);
        if ($tmp===false) {
            return false;
        }
        $tmp = $this->_parse_dot_syntax($tmp);
    ...
    public function ObjectItemAdd($type, $object_id, $items, $pos=0)
    {
        ...
        foreach ($items as $item) {
            $item_list[] = mj_json_encode($item);
        }
        return $this->_cmd('object items add', ['object_id'=>$object_id, 'type'=>$type, 'items'=>implode("\n", $item_list), 'pos'=>$pos]);
    }

    public function ObjectItemRemove($type, $object_id, $items, $item_key)
    {
        return $this->_cmd('object items remove', ['object_id'=>$object_id, 'type'=>$type, 'item_key'=>$item_key, 'items'=>implode("\n", $items)]);
    }

    public function ObjectListGet($type, $object_id, $item_key, $offset, $count, $meta_ids, $filter_deleted=false, $with_categories=false)
    {
        $parameters = ['type'=>$type, 'item_key'=>$item_key, 'object_id'=>$object_id, 'offset'=>$offset, 'count'=>$count];
        if ($with_categories) {
            $parameters['with_categories'] = '1';
        }
        if ($filter_deleted) {
            if (is_array($meta_ids)) {
                $meta_ids[] = -1;
        ...
        if (is_array($meta_ids) && isset($meta_ids[0])) {
            $parameters['meta_ids'] = implode(',', array_keys(array_flip($meta_ids)));
        }
    ...
    public function ObjectCreate($type, $attributes, $sortkey, $public = false)
    {
        if (is_array($attributes)) {
            $attributes = mj_json_encode($attributes);
        }
        return $this->_cmd('object create', [ 'attributes'=>$attributes, 'sort_key'=>$sortkey, 'type'=>$type, 'public'=>$public ]);
    }
    ...
    public function ObjectUpdate($object_id, $attributes, $sortkey, $type, $is_public = false)
    {
        ...
        return $this->_cmd('object update', [ 'object_id'=>$object_id, 'type'=>$type, 'sort_key'=>$sortkey, 'attributes'=>$attributes, 'is_public'=>$is_public]);
    }

    public function ObjectDelete($object_id, $type)
    {
        return $this->_cmd('object delete', ['object_id'=>$object_id, 'type'=>$type]);
    }
    public function ObjectLightboxSend2($rcpt_user_id, $list_id, $expires, $new_list_title, $media_ids, $acl_actions)
    {
        $p = ['user_id'=>$rcpt_user_id, 'list_id'=>$list_id, 'expires'=>$expires, 'media_ids'=>implode(',', $media_ids)];
        foreach ($acl_actions as $media_id=>$actions) {
            $p['acl_action.' . $media_id] = implode(',', $actions);
        }
        $item_list = [];
        foreach ($media_ids as $media_id) {
            $item_list[] = mj_json_encode(['media_id'=>$media_id]);
        }
        $p['item_list'] = implode("\n", $item_list);
        $p['attributes'] = mj_json_encode(['title'=>$new_list_title, 'note'=>'']);
    ...
    {
        $p = ['email'=>$email, 'first_name'=>$first_name, 'last_name'=>$last_name, 'pass'=>$pass, 'expires'=>$expires, 'media_ids'=>implode(',', $media_ids)];
        foreach ($acl_actions as $media_id=>$actions) {
            $p['acl_action.' . $media_id] = implode(',', $actions);
        }
        $item_list = [];
        ...
        $p['item_list'] = implode("\n", $item_list);
        $p['attributes'] = mj_json_encode(['title'=>$lb_title, 'note'=>'']);
        $p['sort_key'] = $lb_title;
    ...
    {
        $p = ['user_id'=>$rcpt_user_id, 'list_id'=>$list_id, 'media_ids'=>implode(',', $media_ids), 'acl_actions'=>implode(',', $acl_actions)];
        $item_list = [];
        foreach ($media_ids as $media_id) {
            $item_list[] = mj_json_encode(['media_id'=>$media_id]);
        }
    ...
    {
        $p = ['object_id'=>$id, 'relation'=>$relation, 'type'=>$type];
        return $this->_cmd('object relation get', $p);
    }
    public function ObjectRelationCheck($id, $relation)
    {
        return $this->_cmd('object relation check', ['object_id'=>$id, 'relation'=>$relation]);
    }
    public function ObjectRelationOptionCheck($id, $relation, $option)
    {
        return $this->_cmd('object relation option check', ['object_id'=>$id, 'relation'=>$relation, 'option'=>$option]);
    }
    public function ObjectRelationDelete($id, $relation, $user_id=false)
    {
        $p = ['object_id'=>$id, 'relation'=>$relation];
        if ($user_id !== false) {
            $p['user_id'] = $user_id;
        }
